...
export class StreamLabsConnexion {
  ...
  load = async () => {
    await this.evntCom.notify("newEvent", [EStreamLabsEvent.LOAD, null, { emitter: this.config.name }]);
    this.socket = io(`https://sockets.streamlabs.com?token=${this.config.token}`, {
    ...
    this.socket.on("connect", () => {
      console.log('connect')
      this.evntCom.notify("newEvent", [EStreamLabsEvent.OPEN, null, { emitter: this.config.name }]);
    });
    ...
    this.socket.on("disconnect", () => {
      this.evntCom.notify("newEvent", [EStreamLabsEvent.CLOSE, null, { emitter: this.config.name }]);
    });
              ...
              switch (type) {
                case "merch":
                  message.forEach((item: any) => {
                    this.evntCom.notify("newEvent", [EStreamLabsEvent.MERCH, {
                      event_id,
                      type,
                      for: referrer,
                      ...item,
                ...
                case "donation":
                  message.forEach((item: any) => {
                    this.evntCom.notify("newEvent", [EStreamLabsEvent.DONATION, {
                      event_id,
                      type,
                      for: referrer,
                      ...item,
